...
export default function AIAnalysis() {
  ...
  const [stickyIds, setStickyIds] = useState<string[]>([]); // keep just-analyzed visible in pending tab
  ...
  const avgRisk = useMemo(() => threats.length ? Math.round(threats.reduce((a,t)=>a+t.risk,0)/threats.length) : 0, [threats]);
  ...
  const runOne = async (threatId: string) => {
    ...
    try {
      ...
      attachAnalysis(threatId, data.analysis);
      setStickyIds(ids => ids.includes(threatId) ? ids : [...ids, threatId]);
      toast.success("AI analysis complete");
    } catch (e: any) {
      const msg = e?.message || "";
      if (msg.includes("429")) toast.error("Rate limited");
      else if (msg.includes("402")) toast.error("AI credits exhausted");
      else toast.error("Analysis failed");
    } finally { setRunning(r => ({...r, [threatId]: false})); }
  };

  const runTop5 = async () => {
    setBulkRunning(true);
    const top = [...threats].filter(t => !t.aiAnalysis).sort((a,b)=>b.risk-a.risk).slice(0,5);
    for (const t of top) { await runOne(t.id); }
    setBulkRunning(false);
    toast.success("Top 5 critical analyzed");
  };
    ...
    <div className="space-y-6">
      <header className="flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground mt-1">Run Gemini-powered analysis on detected threats and act on AI recommendations</p>
        </div>
        <button onClick={runTop5} disabled={bulkRunning}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <Mini label="Threats Analyzed" value={`${analyzed.length} / ${threats.length}`} sub={`${Math.round(analyzed.length/Math.max(1,threats.length)*100)}% coverage`} />
        ...
        {list.slice(0, 30).map(t => (
          <div key={t.id} className="panel p-5 space-y-4">
            <div className="flex flex-wrap items-center gap-3">
              ...
              <span className="ml-auto text-xs mono text-muted-foreground">{new Date(t.ts).toLocaleString()}</span>
            </div>
            <div className="font-semibold">{t.summary}</div>
            <div className="text-xs mono text-muted-foreground">src: {t.src} → {t.dst} :{t.port} {t.protocol}</div>
            ...
            {t.aiAnalysis ? (
              ...
              <div className="rounded-lg border border-dashed border-border/60 p-6 text-center">
                <p className="text-sm text-muted-foreground mb-3">No AI analysis yet for this threat</p>
                <button onClick={() => runOne(t.id)} disabled={running[t.id]}
                  ...
                  {running[t.id] ? <Loader2 className="size-4 animate-spin" /> : <Sparkles className="size-4" />}
...
function Mini({ label, value, sub }: { label: string; value: string | number; sub: string }) {
  return (
    <div className="panel p-5">
      ...
      <div className="text-[11px] uppercase tracking-wider text-muted-foreground mt-1">{sub}</div>
    </div>
  );
}
